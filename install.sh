...
- 至少一人做出对自己不利但符合性格的选择
- 不写"学到教训"式收束
- 声纹可区分：盖住角色名，仍能认出谁在说。每个角色固定一套句长、口头习惯、回避方式
...
AI 剧本最隐蔽的毛病：所有角色共用同一张嘴。句长一样、节奏一样、回避方式一样。

每个主要角色在写第一句台词之前，先定三样：

1. **句长习惯**——有人只说短句；有人绕；有人半句半句往外挤
2. **回避方式**——被问到痛处时，这个人是转移话题、开玩笑、反问，还是沉默？每个角色的回避方式必须不同
3. **一个语言习惯**——重复的口头词、从不说的词、独有的比喻域（用钱打比方的人 vs 用身体打比方的人）

### 声纹测试

写完一场戏，盖住所有角色名，只读台词。分不出谁在说，就是失败——回去重写，不是微调。

### 样例：同一句意思，三张嘴

要表达的都是"我不同意这个计划"：

A（短句型，回避=反问）: Whose idea was this.
B（绕型，回避=玩笑）: No, it's great, it's -- we'll all be very rich in prison.
C（挤型，回避=沉默）: I just -- (beat) When does it start.

C 甚至没说不同意。但三个人一张嘴一个样。
...
8. 声纹混同——盖住角色名分不出谁在说。找出两句可以互换角色而毫无违和的台词，引用它们
